// scripts/backfill_wallets_force.js
...
async function backfillForce() {
  ...
  for (const user of users) {
    ...
    await user.save();
    updated++;
    console.log(`Force-updated wallets for user ${user.email || user.username}`);
  }
  console.log(`Force backfill complete. Updated ${updated} users.`);
  process.exit(0);
}

backfillForce().catch(e => { console.error(e); process.exit(1); });
