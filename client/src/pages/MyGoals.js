  ...
  return (
    <div className="w-full max-w-3xl mx-auto p-4">
      <div className="flex items-center mb-8">
        ...
        <h1 className="text-3xl font-bold text-gold-gradient">My Goals</h1>
        <button
          className="ml-auto bg-gold text-black px-4 py-2 rounded-lg flex items-center gap-2 shadow-lg hover:bg-yellow-400 transition"
      ...
      <div className="grid md:grid-cols-2 gap-6">
        {goals.map((goal) => (
          <div
            key={goal.id}
            className={`glass-card p-6 rounded-xl shadow-lg flex flex-col gap-3 border-l-8 bg-gradient-to-br ${goal.color}`}
          >
            ...
            <div className="flex items-center gap-4">
              <div className="flex flex-col">
                ...
                <span className="font-bold text-lg">{goal.target} {goal.id === 2 ? 'Assets' : '$'}</span>
              </div>
              <div className="flex flex-col">
                <span className="text-sm text-gray-300">Current</span>
                <span className="font-bold text-lg">{goal.current} {goal.id === 2 ? 'Assets' : '$'}</span>
              </div>
              <div className="flex flex-col">
                <span className="text-sm text-gray-300">Deadline</span>
                <span className="font-bold text-lg">{goal.deadline}</span>
              </div>
            </div>
      ...
      {showAdd && (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50">
          <div className="bg-dark p-8 rounded-xl shadow-2xl w-full max-w-md relative">
            ...
            <input
              className="w-full mb-3 p-2 rounded bg-gray-800 text-white border border-gray-700 focus:border-gold outline-none"
            ...
            />
            <button
              className="w-full bg-gold text-black py-2 rounded-lg font-bold hover:bg-yellow-400 transition mt-2"
